...
def fetch_batches(warehouse, item_codes):
	"""Return batch data for given items and warehouse.

	Supports both legacy Stock Ledger Entry batches and the
	Serial and Batch Bundle system introduced in ERPNext v15.
	"""
	...
	sbb_rows = frappe.db.sql(
		"""
            SELECT
                sbb.item_code,
                sbe.batch_no,
                SUM(CASE WHEN sbe.is_outward = 1 THEN -sbe.qty ELSE sbe.qty END) AS batch_qty,
                MAX(b.expiry_date) AS expiry_date,
                MAX(b.manufacturing_date) AS manufacturing_date,
                MAX(b.posa_batch_price) AS posa_batch_price
            ...
            GROUP BY
                sbb.item_code,
                sbe.batch_no
            HAVING SUM(CASE WHEN sbe.is_outward = 1 THEN -sbe.qty ELSE sbe.qty END) > 0
        """,
		(warehouse, item_codes, today),
		as_dict=True,
	)

	rows = sle_rows + sbb_rows
	return [
		{
...
def get_items_details(pos_profile, items_data, price_list=None, customer=None):
	...
	def _get_batches(warehouse, item_codes):
		return fetch_batches(warehouse, item_codes)
...
def get_item_detail(item, doc=None, warehouse=None, price_list=None, company=None):
	...
	if warehouse and item.get("has_batch_no"):
		for batch in fetch_batches(warehouse, (item_code,)):
			batch_no_data.append(
				{
					"batch_no": batch.get("batch_no"),
					"batch_qty": batch.get("batch_qty"),
					"expiry_date": batch.get("expiry_date"),
					"batch_price": batch.get("batch_price"),

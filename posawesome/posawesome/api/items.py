...
def get_item_detail(item, doc=None, warehouse=None, price_list=None, company=None):
    ...
    # If a specific UOM was requested, try fetching a matching Item Price
    if item.get("uom") and price_list:
        uom_price = get_price_for_uom(item_code, price_list, item.get("uom"))
        if uom_price:
            res["price_list_rate"] = flt(uom_price)
            res["rate"] = flt(uom_price)

...
def get_items(
	...
	def _get_items(
		...
		if use_limit_search and search_value:
			...
			or_filters = [["name", "like", f"%{item_code}%"], ["item_name", "like", f"%{item_code}%"]]
...
def get_items_groups():
	return frappe.get_all(
		"Item Group", filters={"is_group": 0}, fields=["name"], limit_page_length=200, order_by="name"
	)
	...
	if selected_currency:
		...
		if invoice_doc.currency != frappe.get_cached_value(
			...
			invoice_doc.base_rounded_total = flt(
				invoice_doc.rounded_total * conversion_rate, invoice_doc.precision("base_rounded_total")
			)
...
def search_invoices_for_return(
	...
	for invoice in invoices_list:
		...
		has_returns = frappe.get_all(
			"Sales Invoice", filters={"return_against": invoice.name, "docstatus": 1}, fields=["name"]
		)
...
def validate_return_items(return_against, items):
	...
	for item in items:
		...
		if item_code not in original_items:
			return {"valid": False, "message": f"Item {item_code} not found in original invoice"}
...
def get_available_currencies():
	...
	return frappe.get_all(
		"Currency", fields=["name", "currency_name"], filters={"enabled": 1}, order_by="currency_name"
	)

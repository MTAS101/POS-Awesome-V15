...
def get_items(
    ...
    def _get_items(
        ...
        try:
            ...
            frappe.log_error(
                f"Error clearing bin_qty_cache: {e!s}", "POS Awesome")
...
def get_sales_person_names():
    ...
    try:
        ...
        print(
            f"Found {len(sales_persons)} sales persons: {json.dumps(sales_persons)}")
        return sales_persons
    except Exception as e:
        print(f"Error fetching sales persons: {e!s}")
        frappe.log_error(
            f"Error fetching sales persons: {e!s}", "POS Sales Person Error")
        return []
...
def validate_return_items(original_invoice_name, return_items):
    ...
    for item in original_invoice.items:
        original_item_qty[item.item_code] = original_item_qty.get(
...
def update_invoice(data):
    ...
    if (data.get("is_return") or invoice_doc.is_return) and invoice_doc.get("return_against"):
        validation = validate_return_items(
            invoice_doc.return_against, [d.as_dict()
    ...
    if selected_currency:
        ...
        if invoice_doc.currency != frappe.get_cached_value(
            ...
            invoice_doc.base_total = flt(
                invoice_doc.total *
                conversion_rate, invoice_doc.precision("base_total")
            )
...
def submit_invoice(invoice, data):
    ...
    if len(mop_cash_list) > 0:
        ...
        cash_account = {"account": frappe.get_value(
        ...
        for row in data.get("customer_credit_dict"):
            if row["type"] == "Advance" and row["credit_to_redeem"]:
                ...
                ensure_child_doctype(
                    invoice_doc, "advances", "Sales Invoice Advance")
...
def redeeming_customer_credit(invoice_doc, data, is_payment_entry, total_cash, cash_account, payments):
    ...
    if data.get("redeemed_customer_credit"):
        ...
        if not cost_center:
            frappe.throw(_("Cost Center is not set in pos profile {}").format(
                invoice_doc.pos_profile))
        for row in data.get("customer_credit_dict"):
            if row["type"] == "Invoice" and row["credit_to_redeem"]:
                ...
                ensure_child_doctype(jv_doc, "accounts",
                    ...
                    for batch in batch_list:
                        if batch.qty > 0 and batch.batch_no:
                            batch_doc = frappe.get_cached_doc(
                                "Batch", batch.batch_no)
                            if (
                                    str(batch_doc.expiry_date) > str(
                                        today) or batch_doc.expiry_date in ["", None]
                            ) and batch_doc.disabled == 0:
                                batch_no_data.append(
                                    {
                                        "batch_no": batch.batch_no,
...
def create_customer(
    ...
    if birthday:
        try:
            ...
            frappe.log_error(
                f"Error formatting birthday: {birthday}", "POS Awesome")
    ...
    for invoice in invoices_list:
        ...
        if has_returns:
            ...
            if filtered_items:
                ...
                filtered_invoice = frappe.get_doc(
...
def build_item_cache(item_code):
    ...
    for row in item_variants_data:
        ...
        attribute_value_item_map.setdefault(
            (attribute, attribute_value), []).append(item_code)
    ...
    frappe.cache().hset("attribute_value_item_map",
                        parent_item_code, attribute_value_item_map)
    frappe.cache().hset("item_attribute_value_map",
                        parent_item_code, item_attribute_value_map)
...
def get_item_attributes(item_code):
    ...
    for a in attributes:
        values = frappe.db.get_all(
            ...
            filters={"parenttype": "Item Attribute",
                     "parent": a.attribute},
            order_by="idx asc",
        )
        a.values = values
...
def make_payment_request(**args):
    ...
    bank_account = (
        get_party_bank_account(args.get("party_type"), args.get(
            "party")) if args.get("party_type") else ""
    )
...
def get_sales_invoice_child_table(sales_invoice, sales_invoice_item):
    ...
    child_doc = frappe.get_doc("Sales Invoice Item", {
...
def get_app_info() -> dict[str, list[dict[str, str]]]:
    ...
    for app_name in installed_apps:
        try:
            ...
            app_version = frappe.get_attr(
                f"{app_name}.__version__") or "Unknown"
        except (AttributeError, ImportError):
            app_version = "Unknown"

        apps_info.append(
            {"app_name": app_name, "installed_version": app_version})

...
def process_pos_payment(payload):
			...
			try:
				...
				try:
					...
					if payment_account:
						...
						frappe.log_error(
							f"Using payment account from mode_of_payment: {bank_account}", "POS Payment Debug"
						)
				except Exception as e:
				...
				if new_payment_entry:
					if not bank_account:
						...
						frappe.log_error(
							f"Using bank account from payment entry: {bank_account}", "POS Payment Debug"
						)
	...
	if len(data.selected_invoices) > 0 and data.total_selected_invoices > 0:
		...
		if total_payment_amount > 0:
			...
			frappe.log_error(
				f"Creating payment allocation with total: {total_payment_amount}", "POS Payment Details"
			)
			...
			if created_payment and new_payment_entry:
				...
				if new_payment_entry.docstatus == 1:
					...
					try:
						...
						frappe.log_error(
							f"Error creating payment entry for allocation: {str(e)}", "POS Payment Error"
						)
					...
					frappe.log_error(
						f"Using existing payment entry: {payment_entry.name}", "POS Payment Debug"
					)
			else:
				# Create a new payment entry for allocation
				try:
			...
			if payment_entry:
				try:
					...
					for invoice in data.selected_invoices:
						...
						if allocation <= 0:
							frappe.log_error(
								f"Zero allocation for invoice {invoice_name}", "POS Payment Debug"
							)
							continue
					...
					if not created_payment or payment_entry.name != new_payment_entry.name:
						frappe.log_error(
							f"Adding new payment entry to results: {payment_entry.name}", "POS Payment Debug"
						)
					...
					if created_payment and new_payment_entry and new_payment_entry.docstatus == 0:
						try:
							...
							frappe.log_error(
								f"Error submitting original payment entry: {str(e)}", "POS Payment Error"
							)
...
def get_party_account(party_type, party, company):
	try:
		...
		if not account:
			frappe.log_error(
				f"No account found for {party_type} {party} in company {company}", "POS Account Error"
			)
...
def create_direct_journal_entry(
	company, customer, invoices_list, payment_amount, bank_account=None, mode_of_payment=None
):
	"""Create a journal entry directly to handle payment allocation and bypass payment entry reconciliation issues"""
	try:
		frappe.log_error(
			f"Creating direct journal entry for {customer} with amount {payment_amount}", "Direct JE Debug"
		)
		...
		if not bank_account:
			frappe.log_error(
				f"Bank account not provided, trying mode_of_payment: {mode_of_payment}", "Direct JE Debug"
			)
			if mode_of_payment:
				...
				if payment_account:
					bank_account = payment_account
					frappe.log_error(
						f"Found payment account from mode_of_payment: {bank_account}", "Direct JE Debug"
					)
				else:
		...
		if len(je.accounts) > 1:  # Need at least 2 entries (bank + receivable)
			...
			frappe.log_error(
				f"JE validation: Total Debit={total_debit}, Total Credit={total_credit}", "Direct JE Debug"
			)

			# Ensure balanced entry
			if abs(total_debit - total_credit) > 0.01:
				frappe.log_error(
					f"Unbalanced JE: debit={total_debit}, credit={total_credit}", "Direct JE Error"
				)
	...
	try:
		...
		if len(error_msg) > 1000:
			for i in range(0, len(error_msg), 1000):
				...
				frappe.log_error(
					f"Error in setup_payment_entry_cancel_hook (part {i // 1000 + 1}): {chunk}", "POS Error"
				)
		else:

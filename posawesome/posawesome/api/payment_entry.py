...
def process_pos_payment(payload):
	...
	# Allocate each payment entry against selected invoices
	if payment_entries and len(data.selected_invoices) > 0 and data.total_selected_invoices > 0:
		...
		for invoice in data.selected_invoices:
			...
			if "outstanding_amount" not in invoice or not invoice.get("outstanding_amount"):
				try:
					si = frappe.get_doc("Sales Invoice", invoice.get("voucher_no"))
					...
					errors.append(
						f"Could not process invoice {invoice.get('voucher_no')}: missing data"
					)

		invoice_queue = data.selected_invoices[:]
		...
		for payment_entry in payment_entries:
			try:
				...
				while remaining_amount > 0 and invoice_queue:
					...
					invoice["outstanding_amount"] = outstanding_amount - allocation
					if invoice["outstanding_amount"] <= 0:
						invoice_queue.pop(0)

				allocated_total = sum(flt(ref.allocated_amount) for ref in payment_entry.references)
				payment_entry.paid_amount = payment_entry.received_amount = allocated_total

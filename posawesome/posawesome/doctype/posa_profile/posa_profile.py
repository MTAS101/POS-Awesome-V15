	...
	def validate_default_profile(self):
		for row in self.applicable_for_users:
			res = frappe.db.sql(
				"""select pf.name
				from
					`tabPOS Profile User` pfu, `tabPOSA Profile` pf
				where
			...
			if row.default and res:
				...
				msgprint(
					_(
						"User {0} doesn't have any default POSA Profile. Check Default at Row {1} for this User."
...
def get_item_groups(pos_profile):
	item_groups = []
	pos_profile = frappe.get_cached_doc("POSA Profile", pos_profile)
...
def pos_profile_query(doctype, txt, searchfield, start, page_len, filters):
	...
	pos_profile = frappe.db.sql(
		"""select pf.name
		from
			`tabPOSA Profile` pf, `tabPOS Profile User` pfu
		where
	...
	if not pos_profile:
		...
		pos_profile = frappe.db.sql(
			"""select pf.name
			from
				`tabPOSA Profile` pf left join `tabPOS Profile User` pfu
			on
				pf.name = pfu.parent
			where
				ifnull(pfu.user, '') = ''
...
def set_default_profile(pos_profile, company):
	...
	if pos_profile and company:
		frappe.db.sql(
			""" update `tabPOS Profile User` pfu, `tabPOSA Profile` pf
			set

...
export default {
	...
	handelOffers() {
		console.log("[debug] handelOffers called", {
	...
	checkQtyAnountOffer(offer, qty, amount) {
		console.log("[debug] checkQtyAnountOffer", {
			offer: offer.name,
			qty,
			amount,
		});
		...
		console.log("[debug] checkQtyAnountOffer result", res);
		return res;
	},
	...
	checkOfferCoupon(offer) {
		if (offer.coupon_based) {
			...
			if (coupon) {
				...
				console.log("[debug] coupon not found for offer", offer.name);
				return false;
			}
		} else {
			offer.coupon = null;
	...
	getItemOffer(offer) {
		...
		if (offer.apply_on === "Item Code") {
			if (this.checkOfferCoupon(offer)) {
				this.items.forEach((item) => {
					console.log("[debug] evaluating item for offer", {
						offer: offer.name,
						item: item.item_code,
						qty: item.stock_qty,
					});
					if (!item.posa_is_offer && item.item_code === offer.item) {
						...
						if (
							...
							if (res.apply) {
								console.log("[debug] offer conditions met", {
									offer: offer.name,
									item: item.item_code,
								});
		...
		if (!apply_offer) {
			console.log("[debug] no item offer applied for", offer.name);
		}
		return apply_offer;
	},
	...
	getGroupOffer(offer) {
		...
		if (offer.apply_on === "Item Group") {
			if (this.checkOfferCoupon(offer)) {
				...
				if (total_count || total_amount) {
					...
					if (res.apply) {
						console.log("[debug] group offer matched", {
							offer: offer.name,
							items: items.length,
						});
						offer.items = items;
		...
		if (!apply_offer) {
			console.log("[debug] no group offer applied for", offer.name);
		}
		return apply_offer;
	},
	...
	getBrandOffer(offer) {
		...
		if (offer.apply_on === "Brand") {
			if (this.checkOfferCoupon(offer)) {
				...
				if (total_count || total_amount) {
					const res = this.checkQtyAnountOffer(offer, total_count, total_amount);
					if (res.apply) {
						console.log("[debug] brand offer matched", {
							offer: offer.name,
							items: items.length,
						});
						offer.items = items;
		...
		if (!apply_offer) {
			console.log("[debug] no brand offer applied for", offer.name);
		}
		return apply_offer;
	},
	getTransactionOffer(offer) {
		...
		if (offer.apply_on === "Transaction") {
			if (this.checkOfferCoupon(offer)) {
				...
				if (total_count || total_amount) {
					const res = this.checkQtyAnountOffer(offer, total_count, total_amount);
					if (res.apply) {
						console.log("[debug] transaction offer matched", {
							offer: offer.name,
		...
		if (!apply_offer) {
			console.log("[debug] no transaction offer applied for", offer.name);
		}
		return apply_offer;
	},
	...
	updateInvoiceOffers(offers) {
		console.log("[debug] updateInvoiceOffers", {

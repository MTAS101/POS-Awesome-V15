...
export function useItemOperations() {
  ...
  const addItem = (item, context) => {
    ...
    if (index === -1 || context.new_line) {
      ...
      context.updateItemDetail(new_item, true);
      ...
      context.updateItemsDetails([cur_item]);
      ...
      context.calcStockQty(cur_item, cur_item.qty);
      ...
      context.setSerialNo(cur_item);
    }
    context.forceUpdate();

    // Only try to expand if new_item exists and should be expanded
    if (new_item && ((!context.pos_profile.posa_auto_set_batch && new_item.has_batch_no) || new_item.has_serial_no)) {
      context.expanded = [new_item.posa_row_id];
    }
  };
  ...
  const clearInvoice = (context) => {
    ...
    context.additional_discount = 0;  // Added for additional discount

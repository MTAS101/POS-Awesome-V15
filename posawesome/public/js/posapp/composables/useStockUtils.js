export function useStockUtils() {
  const calcUom = (item, value, context) => {
    ...
    if (item.posa_offer_applied) {
      ...
      const offer = context.posOffers && Array.isArray(context.posOffers) ? context.posOffers.find(o => {
      ...
      if (offer && offer.discount_type === "Rate") {
        ...
        if (item.original_base_price_list_rate) {
          ...
          updated_base_price = context.flt(item.original_base_price_list_rate * item.conversion_factor, context.currency_precision);
        } else {
          // Fallback if original price not stored
          updated_base_price = context.flt(item.base_price_list_rate * conversion_ratio, context.currency_precision);
        }
        ...
        const base_discount = context.flt((updated_base_price * offer.discount_percentage) / 100, context.currency_precision);
        ...
        if (context.selected_currency !== context.pos_profile.currency) {
          item.price_list_rate = context.flt(updated_base_price / context.exchange_rate, context.currency_precision);
